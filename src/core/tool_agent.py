...
class ToolAgent:
    ...
    def can_run(self):
        # Check budget, if we have exceeded the budget, return False
    ...
    def add_budget(self, budget_dollars):
        self.total_cost -= budget_dollars
    ...
    def track_usage(self, usage):
        ...
        logger.info(
            f"Total usage so far for agent {self.agent_key}: {self.total_cost}"
        )

    def _get_available_fn_defn(self):
    ...
    def _build_message_stack(self):
        system_message = (
            self.text_config.system_message or "You are a helpful assistant."
        )
        ...
        chat_history = (
            self.agent_service.update_context_memory(
                self.object_config.agent_id, memory_elements=[]
            )
            or []
        )
    ...
    def handle_fn_call(self, tool_calls):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            future_to_tool_call = {
                executor.submit(self._execute_function_call, tool_call): tool_call
    ...
    def _execute_function_call(self, tool_call):
        ...
        try:
            ...
            traceback.print_exc()  # Print the stack trace
            logger.warn(
                f"_execute_function_call failed to parse tool call {tool_call}. Error: {e}"
            )
            return json.dumps({"error": f"Invalid function call structure. Error: {e}"})
    ...
    def run(self, input_messages=None, system_message_override=None):
        try:
            ...
            while True:
                ...
                available_function_definitions = self._get_available_fn_defn()
